...
* **Visual Structure:** Dark amber/slate glassmorphic toolbar (`#1E293B` background with `#F59E0B` amber border) embedded at the top of the active monitoring view (`MOB_SLEEP_MONITOR` / `State_MonitoringActive`) on `MeasurementPage` when `DEV_MODE=true`. Displays a header ("⚡ DEV MODE SIMULATOR") and horizontal scrolling action chips: `[Noise Floor Envelope Sample]`, `[Normal (16 bpm)]`, `[In-Band >10s]`, and `[Recovery (5s)]`. Active scenario chip highlights in solid `{colors.warning_amber}` or `{colors.accent_green}`.
