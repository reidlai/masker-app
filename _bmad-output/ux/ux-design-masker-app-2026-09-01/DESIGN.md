...
* **Visual Structure:** Dark amber/slate glassmorphic toolbar (`#1E293B` background with `#F59E0B` amber border) embedded at the top of the active monitoring view (`MOB_SLEEP_MONITOR` / `State_MonitoringActive`) on `MeasurementPage` when `DEV_MODE=true` or master BLE Telemetry Simulator toggle in Settings is enabled. Displays a header ("⚡ DEV SIMULATOR TOOLBAR"), two action buttons (`[Stop Breathing during sleep]`, `[Normal Breathing during sleep]`), and a live **Detection Mechanism Stage Monitor** diagnostic card (`[Stage 1: Normal]` / `[Stage 2: Stop Breathing]` / `[Stage 3: Alert]`).
