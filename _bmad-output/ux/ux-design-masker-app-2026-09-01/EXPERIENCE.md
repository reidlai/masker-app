...
- **"First-run onboarding"** — from the flow picker. `MOB_PASSKEY_AUTH` → `MOB_USER_PROFILE` → `MOB_BLE_PERMISSION_PRIMER` → `MOB_DEVICE_PAIRING` → `MOB_CALIBRATION_STAGE1` → `MOB_CALIBRATION_STAGE2` → `MOB_SLEEP_MONITOR` → `MOB_TIER1_ALARM` → `MOB_HOME`. The `MOB_SLEEP_MONITOR` → `MOB_TIER1_ALARM` step stands in for a breathing-pause event during the night; `MOB_TIER1_ALARM`'s "I'm Safe" dismissal returns to monitoring, which ends the walkthrough on `MOB_HOME` (the morning after).
...
[ Home  (header: "Good morning" · streak line) ]  ── scroll ──
   ...
   4  7-night AHI trend card ─────────tap──►  MOB_HISTORY_FILTER
```
...
* **2 — Last-night summary card** (`HomeSummaryCardOrganism`, hero) — label row (`"LAST NIGHT"` + session date), hero AHI value (`{typography.h1}`, tabular-nums) with a trailing `ShadBadge` status pill — `{colors.accent_green}` "Normal" for AHI < 5, `{colors.warning_amber}` "Moderate" for 5–29, `{colors.danger_red}` "Severe" for AHI ≥ 30 — and a two-stat row: **Duration** (`7h 45m`) and **Apnea events**. Whole card is one hit target with a trailing chevron → `MOB_SLEEP_SUMMARY` for that session. Light haptic, `{colors.pressed_surface}` pressed background.
...
* **No start control.** Starting a night's session lives on the **Monitor** tab, not Home — Home is something you read, not act from. `MOB_SLEEP_MONITOR` gains a pre-session **"Start Sleep Monitoring"** state distinct from its Night Mode lock; tapping it goes straight to active monitoring, with `ThermalCalibrationWizardOrganism` re-running only on the first session or when stored calibration is stale/invalid. `[NOTE FOR UX]` `mob_sleep_monitor.html` still renders only the active Night Mode state; the pre-session Start state is specified but pending a mockup pass.
...
* **Accessibility** — each card exposes one composite label, not per-stat fragments ("Last night: AHI 3.2, Normal, 7 hours 45 minutes, 2 apnea events. Opens full summary." / "D-BAND connected, battery 84 percent, last synced 7:02 AM." / "AHI last 7 nights: 3.4 average, down from 4.1 last week. Opens history."). Any card in an actionable/alert state is announced with its role as a button and its alert text first. The trend chart carries a text alternative equal to the delta line — the bars are decorative.
...
[ Summary  (header: "Morning Sleep Summary" · date · [📅 History] ) ]
   • Session score card — AHI ring + clinical line + status badge (MetricStatCard-family)
...
* **Session score card** — the same two-signal logic as `HomeSummaryCardOrganism` (Component Pattern #10): the badge normally reads the AHI band (`{colors.accent_green}` "Normal range" / amber "Moderate" / red "Severe"), but when the session logged ≥ 1 Tier-1 apnea alarm it switches to the amber **alarm-fired** treatment — amber AHI ring, `"N APNEA ALERT"` badge, and the AHI band demoted to the clinical line (`"AHI 3.2 · normal range"` + `"7h 45m monitoring · 1 'I'm Safe' tap"`). Home and Summary read consistently: if one shows the amber alert, so does the other.
...
* **Respiration Waveform card** — the mini waveform is a **single tap target** (trailing "View details ›" in `{colors.accent_green}`, whole card is the hit area) → `MOB_GRAPH_WAVEFORM` for the pinch/pan interactive timeline and FFT. The card footnote names the affordance so it doesn't read as a static image.
...
* **Behavior:** The hero card on the `MOB_HOME` dashboard. Reads the most recently *finalized* session (`State_MorningSummary` output) and renders its AHI, duration, apnea-event count, and date. If no finalized session exists it renders its empty variant in place (see `State_HomeEmpty`) — decided from session-store state, not a flag — while the rest of the dashboard (greeting, device-status, trend) stays put. The populated card is a single tap target → `MOB_SLEEP_SUMMARY` for that session, light haptic, `{colors.pressed_surface}` pressed background. Does **not** poll; refreshes on tab entry. No loading spinner for the common case — session data is local; a cold read shows the empty variant briefly rather than a skeleton.
* **Alarm-awareness (key behaviour):** the card carries **two independent signals** — *clinical severity* (the AHI band) and *did the alarm fire*. If the session recorded ≥ 1 Tier-1 apnea alarm (`State_ApneaBreach` was reached, whether dismissed by an "I'm Safe" tap or by auto-silence), the card switches to its **alarm-fired override** (DESIGN.md #1a): amber `"N apnea alert(s)"` pill in place of the AHI-severity pill, the severity word demoted to a caption, the apnea-events stat in amber. **This is a product-awareness signal, not a clinical one** — a patient whose siren went off in the night must see that on opening the app the next morning, even when the night's AHI is squarely in the normal range. The override never *upgrades* a genuinely severe night's pill down; if both apply (severe AHI *and* an alarm) the pill reads the alarm and the caption states the severe AHI. Screen-reader label leads with the alarm ("Last night: 1 apnea alert. You tapped I'm Safe once. AHI 3.2, normal range…").
...
7. **`State_MorningSummary`:** Session finalized, rendering AHI score, duration, and interactive Skia GPU charts. If the session reached `State_ApneaBreach` at least once, both this screen's score card and the `MOB_HOME` last-night card carry the **alarm-fired** treatment (amber `"N apnea alert(s)"` badge/pill, AHI band demoted to a caption) regardless of the AHI band — see Component Pattern #10 and DESIGN.md #1a.
...
9. **07:02 AM — Back to Home (`MOB_HOME`):**  
   He backs out to the Home tab — the dashboard he'll open every day now. *"Good morning · 12 nights monitored."* Below it: last night's card (**AHI 3.2 · Normal**, 7h 45m, 2 events); then *"D-BAND connected · 84% · Last sync 7:02 AM"* in calm green — the sensor's already back on the charger and reporting; then the 7-night trend, *"3.4 average · down from 4.1 last week."* He reads it in four seconds, taps nothing, and puts the phone down. (Tonight he'll start the next session from the **Monitor** tab.)
