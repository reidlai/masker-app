...
* **Behavior:** Conditionally rendered on `MeasurementPage` during active nocturnal monitoring (`State_MonitoringActive` on `MOB_SLEEP_MONITOR`) when compile-time flag `DEV_MODE=true` is enabled. Emits simulated background telemetry streams into global `BleTelemetryService` singleton upon tapping scenario chips (`Stop Breathing during sleep`, `Normal Breathing during sleep`). Enables developers to trigger simulated breathing events, apnea alerts, and alarms live while monitoring is actively running.
